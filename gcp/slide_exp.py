...
class Cluster:
    def __init__(
        self, N=20, hash_policy="hot", capacity=-1, hot_alpha=1.0, debug=False
    ):
    ...
    def get_node_api(self, n):
        node_internal_ip = self.config["node_internal_ips"][f"hothash-node-{n+1:02d}"]
    ...
    def query_node(self, n, data_id, op="mix", qid=""):
        try:
            ...
            return requests.get(
                self.get_node_api(n) + "/query",
                params={"op": op, "data_id": data_id, "query_id": qid},
            ).json()
    ...
    def query_hothash(self, key, op="mix", qid="", history=None):
        self.stats[key] += 1
        self.cost["num_queries"] += 1
        n = self.get_hothash_node(key, qid=qid, history=history)
        return self.query_node(n, key, op=op, qid=qid)

    def query(self, key, op="mix", qid="", history=None):
        if self.hash_policy == "hot":
            return self.query_hothash(key, op=op, qid=qid, history=history)
    ...
    def profile(self):
        ...
        imbalance = (
            sigma * self.N / self.cost["num_queries"]
            if self.cost["num_queries"] > 0
            else 0.0
        )
...
def generate_workload(
    ...
    pivots = [
        int(i * num_time_steps / (hotness_change + 1))
        for i in range(hotness_change + 2)
    ][1:-1]
    for t in range(N):
    ...
    for t in range(num_time_steps):
        ...
        if mode == "uniform":
            ...
            idx = bisect.bisect_right(pivots, t)
            data_ids = [
                Hash(x + idx) % num_data for x in np.random.zipf(skewness, num_queries)
            ]
        elif mode == "ssb":
            bucket_ids = [Hash(x) % 7 for x in np.random.zipf(skewness, num_queries)]
            all_data_ids = [
                np.random.choice(lodate[bucket_id])
        ...
        for i in range(num_queries):
            data_time_step.append((f"Q{query_id:06d}", f"D{data_ids[i]:03d}", "mix"))
            query_id += 1
...
def execute_workload(
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        sample, sample_tot = defaultdict(int), 0
        for query in workload[0]:
            sample[query[1]] += 1
            sample_tot += 1
        for key in sample:
            sample[key] = round(sample[key] / sample_tot, 2)
        frequency = None
        for t, queries in enumerate(TQDM(workload)):
            curr, curr_tot = defaultdict(int), 0
            for query in workload[t]:
                curr[query[1]] += 1
                curr_tot += 1
            for key in curr:
                curr[key] = round(curr[key] / curr_tot, 2)
            frequency = sample if frequency is None else frequency
            keys = sorted(set(sample) | set(curr))
            sample_values = [sample.get(k, 0) for k in keys]
            curr_values = [curr.get(k, 0) for k in keys]
            correlation = np.corrcoef(sample_values, curr_values)[0, 1]
            if correlation < 0.5:
                for key in frequency:
                    frequency[key] = round((sample[key] + curr[key]) / 2, 2)
                sample = curr
                sample_tot = curr_tot
            for q, d, o in queries:
                local_times[q] = time.time()
                future = executor.submit(
                    query_wrapper, {"qid": q, "key": d, "op": o, "history": frequency}
                )
    ...
    profile = {}
    profile["avg_latency"] = sum(r["local_time"] for r in results.values()) / len(
        results
    )
...
if __name__ == "__main__":
    args = HeavenArguments.from_parser(
        descriptors=[
            ...
            LiteralArgumentDescriptor(
                "H",
                default="hot",
                choices=["hot", "balanced", "bounded", "cons"],
                help="Hash policy",
            ),
            FloatArgumentDescriptor("a", default=1.00, help="Hot alpha"),
            FloatArgumentDescriptor("e", default=0.300, help="Balanced epsilon"),
            IntArgumentDescriptor("X", default=0, help="Number of hotness change"),
            IntArgumentDescriptor(
                "W", default=0, help="Number of time steps for sliding window"
            ),
    ...
    cluster = Cluster(
        N=args.N,
        hash_policy=args.H,
        capacity={"hot": -1, "balanced": C, "bounded": C, "cons": -1}[args.H],
        hot_alpha=args.a,
        debug=args.d,
    )
