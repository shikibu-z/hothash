...
class Cluster:
    ...
    def get_hothash_node_churn(self, key, qid=""):
        ...
        if self.dyn_op:
            frequency = self.dyn_stats[key] / self.dyn_cost["num_queries"]
        num_nodes = min(
            max(int(ceil((frequency**self.alpha) * (self.N - len(self.disabled)))), 1),
    ...
    def get_hothash_node_vring(self, key, qid=""):
        ...
        nodes_set = set(
            [
                n
                for _, n in self._get_hash_ring(salt=("arc", self.vring_map[key]))[
                    :num_nodes
                ]
            ]
        )
    ...
    def query(self, key, op="mix", qid="", hotkeys=[], spore_rf=1):
        if self.hash_policy == "hot":
            ...
            return self.query_sporehash(
                key, op=op, qid=qid, hotkeys=hotkeys, spore_rf=spore_rf
            )
        else:
        ...
        def warmup_single_node(node_idx):
            ...
            for data_item in cached_items:
                result = self.query_node(
                    n=node_idx, data_id=data_item, op="mix", qid="init_cache"
                )
...
def generate_workload(
    ...
    for t in range(num_time_steps):
        ...
        if mode == "uniform":
            ...
            data_ids = [
                Hash(x) % num_data for x in np.random.zipf(skewness, num_queries)
            ]
        elif mode == "ssb":
...
if __name__ == "__main__":
    ...
    if args.O and (args.H == "bounded" or args.H == "balanced"):
        cluster.capacity = max(
            int(ceil(cluster.dyn_cost["num_queries"] / args.N * (1 + args.e))), 1
        )
