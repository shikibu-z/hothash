...
def Hash(key, salt=None):
    return int(
        hashlib.md5(
            (f"{key}" if salt is None else f"{key}|{salt}").encode("utf-8")
        ).hexdigest(),
        16,
    )
...
class RLScheduler:
    ...
    def __init__(self):
        ...
        self.cluster.load_state("cluster_state_exp.json", format="json")
        ...
        self.scheduler.load_checkpoint("rl_scheduler_checkpoint_exp.pth")
...
class SimulationCluster:
    ...
    def __init__(self, num_nodes=20, cache_size=8, num_data=15):
        ...
        # 150MB
        # self.data_items = [f"SD{i:04d}" for i in range(num_data)]
        # 10MB
        # self.data_items = [f"TD{i:05d}" for i in range(num_data)]
        self.node_ids = [f"hothash-node-{i:02d}" for i in range(self.num_nodes)]
    ...
    def query_node(self, n, data_id, op=None, qid=None):
        ...
        return {"cache_miss": cache_miss, "cache_replacement": cache_replacement}
    ...
    def get_node_load(self, i):
        ...
        if 0 <= i < self.num_nodes:
            ...
            raise IndexError(f"Node index {i} out of range (0-{self.num_nodes - 1})")
    ...
    def save_state(self, filepath="cluster_state.json", format="json"):
        ...
        state_data = {
            ...
            "caches": [
                list(cache.keys()) for cache in self.caches
...
class ClusterEnvironment:
    ...
    def _initialize_data_distribution(self):
        ...
        for data_item in self.data_items:
            ...
            self.cluster.query_node(
                n=node_idx, data_id=data_item, op="mix", qid=f"init_cache"
            )
        print("Data distribution initialized.")

    def get_stable_state(
        self, node_infos, normalized_loads, query: Query
    ) -> np.ndarray:
        ...
        node_cache_full = [
            (
                1
                if query_data_distribution[i] == 0
                and len(node_info["cached_keys"]) >= self.cache_size
                else 0
            )
    ...
    def get_state(self, query: Query) -> np.ndarray:
        ...
        node_infos = [self.cluster.get_node_info(i) for i in range(self.num_nodes)]
    ...
    def step(self, query: Query, action: int) -> Tuple[float, bool]:
        ...
        result = self.cluster.query_node(
            n=action, data_id=query.data_items[0], op=query.op, qid=query.id
        )
        ...
        self.cache_replacement_history.append(self.get_cache_replacement(result))
        self.query_history.append(
            {
                "query_id": query.id,
                "node_id": node_id,
                "data_item": query.data_items[0],
                "locality_score": locality_score,
                "action_balance_score": global_load_std,
                "cache_replacement": self.get_cache_replacement(result),
                "reward": reward,
            }
        )
...
class RLQueryScheduler:
    ...
    def __init__(self, environment: ClusterEnvironment, learning_rate: float = 0.001):
...
class QueryGenerator:
    ...
    def __init__(self, mode="ycsb", num_data=15, skewness=1.3, seed=42, dyn_op=False):
        """
        ...
        if dyn_op:
            ...
            # 150MB
            # self.keys = [f"SD{d:04d}" for d in range(num_data)]
            # 10MB
            # self.keys = [f"TD{d:05d}" for d in range(num_data)]
            self.key2op = {key: np.random.choice(self.ops) for key in self.keys}
    ...
    def generate_query(self) -> Query:
        ...
        # 150MB
        # data_item = f"SD{data_id:04d}"
        # 10MB
        # data_item = f"TD{data_id:05d}"
...
def train_scheduler(
    ...
    query_generator = QueryGenerator(num_data=num_data, skewness=skewness)
    ...
    for episode in range(episodes):
        ...
        if env.action_balance_history:
            action_balance_scores.append(
                np.mean(list(env.action_balance_history)[-num_queries_per_episode:])
            )
        if env.global_load_imbalance_history:
            global_load_imbalance.append(
                np.mean(
                    list(env.global_load_imbalance_history)[-num_queries_per_episode:]
                )
            )
        if env.cache_replacement_history:
            cache_replacement = [
                np.var(list(env.cache_replacement_history)[-num_queries_per_episode:])
            ]

        # Print progress
        if episode % 1 == 0:
            avg_reward = np.mean(episode_rewards[-1:]) if episode_rewards else 0
            avg_locality = np.mean(locality_scores[-1:]) if locality_scores else 0
            avg_action_balance = (
                np.mean(action_balance_scores[-1:]) if action_balance_scores else 0
            )
            avg_load_var = (
                np.mean(global_load_imbalance[-1:]) if global_load_imbalance else 0
            )
            avg_cache_replacement = (
                np.mean(cache_replacement[-1:]) if cache_replacement else 0
            )
...
def test_scheduler(
    ...
    node_infos = [
        environment.cluster.get_node_info(i) for i in range(environment.num_nodes)
    ]
    ...
    for i in range(20000):
        ...
        # Get current state
        state = environment.get_stable_state(node_infos, normalized_loads_init, query)
        ...
        # Execute action
        result = environment.cluster.query_node(
            n=action, data_id=query.data_items[0], op=query.op, qid=query.id
        )
    ...
    print(f"Mean global load std during execution: {np.mean(global_load_stds):.4f}")

    print("\n=== Node Load Distribution ===")
    for node_id, load in enumerate(node_loads):
        print(f"Node {node_id}: {load} queries ({load/queries_processed*100:.2f}%)")
...
def plot_training_metrics(training_metrics):
    ...
    combined_score = [
        0.4 * loc + 0.3 * (1 - var) + 0.3 * bal
        for loc, var, bal in zip(
            training_metrics["locality"],
            training_metrics["load_variance"],
...
if __name__ == "__main__":
    ...
    args = HeavenArguments.from_parser(
        descriptors=[
            ...
            IntArgumentDescriptor("T", default=1000, help="Number of query time steps"),
            IntArgumentDescriptor(
    ...
    cluster = SimulationCluster(num_data=args.D)
    cluster.reset()
    ...
    cluster.save_state("cluster_state_exp.json", format="json")

    print("Saving RL scheduler checkpoint...")
    scheduler.save_checkpoint("rl_scheduler_checkpoint_exp.pth")

    print("Training completed. Testing the scheduler...")
    test_scheduler(scheduler, env, num_data=args.D, skewness=args.k)
    ...
    print(
        "Average action balance score: {:.4f}".format(
            sum(env.action_balance_history) / len(env.action_balance_history)
        )
    )

    print(
        "Average global load imbalance: {:.4f}".format(
    ...
    print(
        "Cache replacement rate: {:.4f}%".format(
            env.cache_replacement_history.count(1)
            / len(env.cache_replacement_history)
            * 100
        )
    )
