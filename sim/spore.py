...
class TestPolicy(DefaultPolicy):
    ...
    def manage_tasks(self, timestep, nodes, idle_tasks, running_tasks, ch_ring):
        ...
        assignments = ch_ring.assign_task_spore(idle_tasks)
        return assignments
...
def run_trial(trial_id):
    config = SimControllerConfig(compute_node_count=NODE_COUNT)
...
for log in cache_hit:
    ...
    for i in log:
        if i == True:
            hit += 1
    rate += hit / len(log)
print("Average cache hit rate:", rate / (NODE_COUNT - empty))
print("Average task on node:", stat.mean(task_hist))
print("Deviation:", stat.stdev(task_hist))
print("Raw:", task_hist)
